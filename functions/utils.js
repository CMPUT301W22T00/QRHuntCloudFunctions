...
module.exports = {
    findUserRefWithCode: async function (qrId, score, geoHash, excludedUser) {
    ...
    getAllUserCodeSnapshots: async function (userId) {
        let docsRef = await db.collection(`users/${userId}/qrCodes`).orderBy("score", "desc").get();
        return docsRef.docs;
    },
    getBestUniqueSnapshot: async function (qrCodesSnapshot, excludedQrId) {
    ...
    getBestUniqueForUser: async function (otherUserId) {
    ...
    getDataForQrId: function (qrId, qrCodes) {
